...
Meant as a simple tool to hack things together when you want to leverage python's libs. If you just need HTTP(S) one-liner, user `curl` instead. This scripts is more useful if you need a combination of protocol as well as the ability to script request/responses with Python.
...
def getArgs():
    ...
    parser.add_argument('-s',
                        ...
                        type=str,
                        default="tcp",
                        help="tcp, udp, http or https")
    parser.add_argument('-t',
                        '--target',
                        type=str,
                        default="127.0.0.1",
                        help="Target IP or FQDN.")
    ...
    parser.add_argument('-b',
                        ...
                        help="Payload as a string. Space separated is fine.")
    parser.add_argument('-f',
                        '--file_payload',
                        type=str,
                        default=None,
                        help="File name that is loaded and sent as payload.")
    #parser.add_argument('--file_payload_lines', type = str, default = None, help="")
    parser.add_argument('--fuzz_payload',
                        action="store_true",
                        default=False,
                        help="")
    parser.add_argument('--special',
                        action="store_true",
                        default=False,
                        help="Run some hard-coded function.")
    ...
    while True:
        try:
            ...
            while 1:
                data = conn.recv(
                    1024
                )  # if < 1024, this still returns with whatever was sent in the frame.
                data = data.decode()
                allData.append(data)
                print('Recv chunk:\n', data)
                if len(allData) > 1:
                    print('All %i chunks collated:\n%s' %
                          (len(allData), "".join(allData)))
