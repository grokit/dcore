...
def getArgs():
    ...
    parser.add_argument(
        ...
        '--access_token',
        type=str,
        default=pickRandomToken('randomToken_'),
        help=
        'You need to have this token in order to be able to access the https website.'
    )
    parser.add_argument('-p', '--port', type=int, default=4443)
    parser.add_argument(
        '-s',
        '--share_root_directory',
        default='.',
        type=str,
        help='You share everything in that folder and all sub-folders.')
    parser.add_argument('-c', '--cert_file', type=str, default=None)
    parser.add_argument('-o',
                        '--output_folder',
                        default='.',
                        type=str,
                        help='Where the logs / uploaded files go.')
    ...
    if not os.path.isdir(args.share_root_directory):
        raise BaseException(
            "Invalid directory for args.share_root_directory: %s." %
            args.share_root_directory)

    if args.cert_file is None:
        args.cert_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            'default_server_cert.pem')
...
if __name__ == '__main__':
    ...
    dataToServer = {
        ...
        'authorized_folder': args.share_root_directory,
