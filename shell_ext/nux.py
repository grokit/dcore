...
def screenshot():
    ...
    if False:
        osExec(
            'scrot -s -e \'xclip -selection clipboard -t "image/png" < $f\' /tmp/scrot_latest.png'
        )
...
def hosts():
    ...
    osExec(
        'sudo vi /etc/hosts && sudo ifconfig wlp1s0 down && sudo ifconfig wlp1s0 up'
    )
...
if __name__ == '__main__':
    ...
    fnHash = {
        fnName: fn
        for fnName, fn in inspect.getmembers(sys.modules[__name__])
        if inspect.isfunction(fn) and fnName != 'getArgs'
    }
